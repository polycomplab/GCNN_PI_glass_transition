...
config_name="config_finetune_2023_PI_exp_last_simple_subset_mixed_with_pretrained_on_1K.py"
...
while :
do
    device_idx=${devices[i]}  # fixme: not in a loop - don't use if already busy
    # echo "i=$i device_idx=$device_idx"
    ...
    if (( ${#pids[@]}==4 )); then
        ...
        do
            ...
            then  # reuse gpu
                ...
                break
            # else
                # echo "pid $pid is still alive"
            fi
        done
    else
        i=$(($i + 1))
        i=$(($i % ${#devices[@]}))
        # echo "i={$i} (incremented)"
    fi
    
    counter=$(($counter+1))
    if [ "$counter" == 20 ]; then
        # echo "counted done: $counter. Waiting for the rest to finish"
        wait
        break
    fi
done
