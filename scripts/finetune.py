...
from data.datasets import k_fold_split, k_fold_split_fixed
...
def update_finetune_config(args, config):
    ...
    config.name = f'{config.finetune.epochs}_epochs'\
...
def finetune(config):
        ...
        with torch.no_grad():
            for i, test_data in enumerate(test_loader):
                ...
                if save_preds:
                    ...
                    preds = (pred*data_std+data_mean).cpu().numpy().tolist()
                    data_batch = {
                        "ID": mol_ids,
                        f"{target_name}, pred": preds}
                    # print(data_batch)
    ...
    def train_on_split(train_dataset, val_dataset, test_dataset,
        ...
        for epoch in progressbar:
            ...
            for data in train_loader:
                ...
                train_MAE += F.l1_loss(pred.squeeze(), data.y, reduction='sum').item()
                loss.mean().backward()
                # optimizer.step()
                # optimizer.zero_grad()
    ...
    #     in enumerate(k_fold_split_fixed(dataset, config.finetune.n_splits)):
        ...
        model.load_state_dict(state_dict)  # TODO check if state_dict has changed
