...
# checkpoints_dir = 'checkpoints/finetune/copy_finetuned_on_PI_exp_old_without_pretrain_without_fixed_split/'
checkpoints_dir = 'checkpoints/finetune/copy_finetune_on_PI_exp_new_07_03_2023_without_pretrain_without_fixed_split/'
...
data_splits = list(split_fn(new_dataset, n_splits))
...
old_ids_smiles = old_dataset_loader.dataset.ids_smiles
old_ids, old_smiles = zip(*old_ids_smiles)

normalized_old_smiles = []
for smiles_idx, smiles in enumerate(old_smiles):
    smiles = list(smiles)
    ...
    smiles_normalized = Chem.MolToSmiles(mol)
    normalized_old_smiles.append(smiles_normalized)
old_smiles = normalized_old_smiles
...
for checkpoint_name in sorted(os.listdir(checkpoints_dir)):
    ...
    new_ids_smiles = train_dataset.ids_smiles
    # old_ids, old_smiles = zip(*old_ids_smiles)
    new_ids, new_smiles = zip(*new_ids_smiles)

    normalized_new_smiles = []
    for smiles in new_smiles:
        ...
        mol = Chem.MolFromSmiles(smiles)
        smiles_normalized = Chem.MolToSmiles(mol)
        normalized_new_smiles.append(smiles_normalized)
    new_smiles = normalized_new_smiles

    # intersection = set(old_smiles).intersection(set(new_smiles))
    # print('len(intersection)', len(intersection))
    bad_count = 0
    with torch.no_grad():
        for i, data in enumerate(old_dataset_loader):
            data = data.to(device)
            mol_id = int(data.index.item())

            # matching by normalized_smiles: smiles->rdkit_mol->normalized_smiles

            # find SMILES by id
            mol_smiles = None
            # for id_, smiles in new_ids_smiles:
            # for id_, smiles in old_ids_smiles:
            for smiles_idx, smiles in enumerate(old_smiles):
            # for smiles_idx, smiles in enumerate(new_smiles):
                id_ = old_ids[smiles_idx]
                if mol_id == int(id_):
                    mol_smiles = smiles
                    break
...
results_save_path = f'{checkpoints_dir}preds_on_normalized_smiles_PI_exp_old_after_trained_on_PI_exp_new_07_03_2023.csv'
