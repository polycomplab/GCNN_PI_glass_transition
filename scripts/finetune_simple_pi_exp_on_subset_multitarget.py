...
def finetune_on_exp_simple_on_subset_multitarget(config):
    t1 = time.time()
    ...
    train_targets = config.finetune.train_targets  # target chemical properties
    if train_targets is not None and train_targets != "None":  # TODO
        train_targets_split = train_targets.split('_pretrained_on_')
        # if len(train_targets_split) == 1:
        #     cp_load_path = None
        # else:
    ...
    target_name = dataset.target_name  # NOTE Tg
    save_preds = config.finetune.save_preds
    if save_preds:
        data_format = {"ID": [], f"{target_name}, pred": []}
        all_preds_df = pd.DataFrame(data_format)
    ...
    log_path = os.path.join(config.checkpoints_dir, 'tb_logs_finetune_on_exp_data_full_splits')
    ...
    # )
    cp_savedir = os.path.join(config.checkpoints_dir, 'finetunes_on_exp_full_splits')
    os.makedirs(cp_savedir, exist_ok=True)

    cp_save_prefix = os.path.join(cp_savedir,
    ...
    for split_idx, (train_dataset, val_dataset, test_dataset) \
        ...
        all_preds_df = all_preds_df.append(
            preds_df, ignore_index = True)
        
        cp_save_path = f'{cp_save_prefix}_split_{split_idx}_last.pth'
        torch.save({
    ...
    if save_preds:
        preds_save_path = f'{cp_save_prefix}_all_splits_final.csv'
        all_preds_df.to_csv(preds_save_path, index=False)
...
def train_on_subset(*, train_loader, test_loader, log_path, scheduler,
                    ...
                    model, optimizer, save_preds, #target_names,
                    cp_save_prefix, split_idx):
    ...
    for epoch in range(1, config.finetune.epochs+1):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            target = (target - dataset_means)/dataset_stds
        ...
        if epoch == config.finetune.epochs:  # training ended
            csv_dict = {}
            # 'test_loss': test_loss
            csv_dict[f'Test_Loss_MSE_{tgt_name}'] = [float(best_test_loss)]
            csv_dict[f'Test_RMSE_{tgt_name}'] = [float(best_test_RMSE)]
            csv_dict[f'Test_R2_{tgt_name}'] = [float(best_test_R2)]
            csv_dict[f'Test_MAE_{tgt_name}'] = [float(best_test_MAE)]
            csv_df = pd.DataFrame(csv_dict)
            save_path = f'{cp_save_prefix}_split_{split_idx}_best.csv'
...
def evaluate(*, test_loader, save_preds, model,
    ...
    test_RMSE = torch.zeros(1, device=device)
    ...
    target_name = test_loader.dataset.target_name  # NOTE Tg
    if save_preds:
        data_format = {"ID": [], f"{target_name}, pred": []}
    ...
    with torch.no_grad():
        for i, test_data in enumerate(test_loader):
            ...
            # pred_denormalized = pred*dataset_stds_tensor+dataset_means_tensor
            pred_denormalized = pred*dataset_stds+dataset_means
            # if i == len(test_loader)-1:
            ...
            # target_original = target_original[range_mask]
            ...
            if save_preds:
                ...
                pred_denormalized_logged = pred_denormalized.clone().cpu().numpy().tolist()
                data_batch = {
                    "ID": mol_ids,
                    f"{target_name}, pred": pred_denormalized_logged}  # FIXME add log for perms
                # print(data_batch)
