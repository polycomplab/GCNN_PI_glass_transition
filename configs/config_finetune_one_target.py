...
if pre_type is None:
    base_name = f"1 target perm: finetune without pretrain "
else:
    base_name = f"1 target perm: finetuning weights pretrained on {pre_type}"
...
model = KGNNModel(num_targets=1, num_task_specific_layers=2)
...
if finetune.state:
    ...
    finetune.target_name = 'He, Barrer'  # TODO group dataset path and target name
    ...
    # finetune.dataset2 = ExperimentalDataset(root=dataset_path, target_name=finetune.target_name)
