...
pretrains = {
        ...
        '700_just_trained': ('700_just_trained', '60_epochs_lr_0.0006_round_67_PI_Syn_subset_1Kfinetuning on PI_Syn_subset_1K#Fri Oct 27 21:45:28 2023_simple_subset_700_last.pth'),
...
prefix = 'round_69'#_really_old_PI_exp_db'

# dataset_path = "datasets/PA_exp"
# dataset_path = "datasets/PI_exp"
# dataset_path = "datasets/PI_exp_new_10_11_2022"
dataset1_path = "datasets/PI_exp_new_07_03_2023"
dataset2_path = "datasets/PI_Syn_subset_1K"
...
# prefix = prefix + f'_{finetune_dataset1_name}_mixed_with_{finetune_dataset2_name}'
...
base_name = f"finetuning on" + f'{finetune_dataset1_name}_mixed_with_{finetune_dataset2_name}'
...
if finetune.state:
    from data.datasets import ExperimentalDataset, MixedTrainDatasetV2
    ...
    # finetune.subset_size = 1_000  # NOTE
    # finetune.subset_size = 700
    # finetune.test_subset_size = 300
    finetune.test_subset_size = 700
    ...
    dataset1 = ExperimentalDataset(root=dataset1_path,
                                   target_name=finetune.target_name)
    train_dataset1, test_dataset = \
        split_train_val(dataset1, test_size=finetune.test_subset_size)
    
    # train_dataset1_size = 300
    # train_dataset1 = split_subindex(train_dataset1, train_dataset1_size)

    dataset2 = ExperimentalDataset(root=dataset2_path, target_name=finetune.target_name)
    finetune.train_dataset = MixedTrainDatasetV2(train_dataset1, dataset2)
    finetune.test_dataset = test_dataset
    # if pre_type is None:
    ...
    # finetune.optimizer_params = {"lr": 1e-3, "momentum": 0.9, "weight_decay": 1e-4}
    # finetune.optimizer_params = {"lr": 6e-4, "weight_decay": 1e-4}
    # finetune.optimizer_params = {"lr": 1e-3, "momentum": 0.9, "weight_decay": 1e-4}
